...
4. **Servidor de Almacenamiento (Zona LAN):**
...
* **Servidor Principal:** Ejecuta cargas de trabajo híbridas para maximizar el rendimiento y la fiabilidad:
  ...
  * Capa Orquestación (Clúster K3s): Ejecuta la infraestructura de gestión corporativa mediante Deployments y Services:
    1. Gestión Taller (GLPI / Leantime): Sistema de tickets y facturación.
    2. OpenLDAP: Directorio central de usuarios.
    3. Base de Datos (MariaDB): Almacena los registros de GLPI y LDAP, utilizando Persistent Volume Claims (PVC) conectados al NAS.
    4. Ingress Controller (Traefik): Enruta el tráfico interno hacia los contenedores correctos.
...
1. **Preservación y Rendimiento:** Lograr una emulación sin latencia instalando RomM en formato Bare Metal.
2. **Alta Disponibilidad (DevOps):** Implementar Kubernetes (K3s) para la gestión empresarial. Si el contenedor de GLPI o la Base de Datos falla, el orquestador detectará el error y levantará un nuevo Pod automáticamente en segundos.
3. **Teletrabajo y Seguridad:** Desplegar una VPN corporativa con WireGuard para administrar remotamente el clúster y el firewall.
4. **Aislamiento Comercial:** Mantener la web pública en una DMZ estrictamente separada de los datos de los clientes.
...
| Componente Lógico | Solución Elegida | Coste | Solución Propietaria | Ahorro | 
| ----- | ----- | ----- | ----- | ----- | 
| Firewall y VPN | pfSense + WireGuard | 0 € | Fortinet + AnyConnect | > 800 € / año | 
| Orquestación | Kubernetes (K3s) | 0 € | VMware Tanzu / OpenShift | > 1.500 € | 
| Sistema de Identidad | OpenLDAP | 0 € | Win Server + AD CALS | > 800 € | 
| Gestión Taller (ERP) | GLPI / Leantime | 0 € | ServiceNow / Jira | Suscripción | 
